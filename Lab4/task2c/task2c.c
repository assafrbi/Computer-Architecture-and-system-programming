...
char c, buf[BUF_SIZE], d_type, outputExitMsg[24] = "Can not open output file", inputExitMsg[23] = "Can not open input file";
char dirExitMsg[23] = "Can not open directory" , getdentesExitMsg[27] = "Can not read with getdents\n";
char* retVal;
char* first_char_arg;
int* a;
ent *d;
...
void print_address(){
    char* address = itoa((int)code_start);
    system_call(SYS_WRITE, STDOUT , "code_start's address: " , 22);
    system_call(SYS_WRITE, STDOUT , address , strlen(address));
    system_call(SYS_WRITE, STDERR, "\n", 1);
    address =  itoa((int)code_end);
    system_call(SYS_WRITE, STDOUT , "code_end's address: " , 20);
    system_call(SYS_WRITE, STDOUT , address , strlen(address));
    system_call(SYS_WRITE, STDERR, "\n", 1);
}
