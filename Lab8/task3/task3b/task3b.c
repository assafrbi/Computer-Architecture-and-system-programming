...
void Print_Section_Names(state* s){
    ...
    char* string_table = (char*)(map_start + (section[header->e_shstrndx]).sh_offset); 
    if(s->debug_mode){
...
void Relocation_Tables (state* s){
    ...
    char* str_table;
    for(int i =0; i<header->e_shnum; i++)
        if(section[i].sh_type == SHT_STRTAB && strcmp(".dynstr", &sec_head_str_table[section[i].sh_name])==0)
            str_table = (char*) map_start+section[i].sh_offset;
    for(int i =0; i<header->e_shnum; i++)
    ...
    for(int i = 0; i< header->e_shnum; i++)
        if(section[i].sh_type == SHT_REL){
            ...
            int size = section[i].sh_size/section[i].sh_entsize;
            printf("Relocation section '%s' at offset ox%x contains %d entries:\n", &sec_head_str_table[section[i].sh_name], section[i].sh_offset, size);
            printf(" Offset\t  Info    Type\n");
            for(int i = 0 ; i < size ; i++){
                printf("%08x %08x  %d\t", relocation[i].r_offset, relocation[i].r_info, ELF32_R_TYPE(relocation[i].r_info));
                int index = ELF32_R_SYM(relocation[i].r_info);
                if(index != 0)
                    printf("%08x\t%s",symbol[index].st_value, &str_table[symbol[index].st_name]);
                printf("\n");
            }
            printf("\n");
        }
